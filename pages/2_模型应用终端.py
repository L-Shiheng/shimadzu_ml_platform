...
try:
    ...
    class MassSpecFeatureSelector(BaseEstimator, TransformerMixin):
        pass # 占位，依赖 joblib 自身的反序列化恢复状态

# --- 辅助函数：将高清 Matplotlib 图表转为 Base64 编码，用于嵌入 HTML ---
...
if model_file and data_file:
    ...
    with col_label:
        has_labels = st.checkbox("本次上传的数据包含真实标签 (用于计算准确率和混淆矩阵)", value=True)
        if has_labels:
    ...
    if st.button("🚀 开始预测并生成临床报告", type="primary"):
        with st.spinner("正在执行脏数据清洗、特征提取、预测及 SHAP 解析..."):
            try:
                ...
                # ---------------- SHAP 可解释性分析 ----------------
                # SHAP 原生支持树模型，对深度学习支持较慢，这里做智能路由
                ...
                if model_type in ['XGBClassifier', 'RandomForestClassifier']:
                    ...
                    # 绘制高清 SHAP Summary Plot (300 dpi)
                    fig_shap, ax_shap = plt.subplots(figsize=(10, 8), dpi=300)
                    # XGBoost 二分类可能返回单个矩阵或列表，这里做兼容处理
                    if isinstance(shap_values, list):
                        shap_val_to_plot = shap_values[1] # 取正类的 SHAP
                    else:
                    ...
                    html_plots['shap_summary'] = None # SVM 或 DNN 暂不强制运行耗时的 SHAP
                ...
                if has_labels:
                    ...
                    with col_plot2:
                        if html_plots.get('shap_summary'):
                            ...
                            st.info(f"当前模型引擎 ({model_type}) 不属于树模型，已跳过耗时的 SHAP 运算。")
                else:
                    if html_plots.get('shap_summary'):
                ...
                if has_labels:
                    ...
                    html_template += f"<p><b>Test Set Accuracy:</b> {st.session_state['acc']:.2%}</p>"
                    html_template += f"""
                    <div class="img-container">
                        <h3>Confusion Matrix (300 DPI)</h3>
                        <img src="data:image/png;base64,{html_plots['confusion_matrix']}" alt="Confusion Matrix">
                    </div>
                    """
                ...
                if html_plots.get('shap_summary'):
                    ...
                    <div class="img-container">
                        <img src="data:image/png;base64,{html_plots['shap_summary']}" alt="SHAP Summary">
                    </div>
                    """
                    
                html_template += f"""
                    <h2>3. Detailed Prediction Results</h2>
                ...
                with col_d2:
                    st.download_button(
                        label="📑 下载高清临床诊断报告 (HTML, 内部图片>=300dpi)",
                        data=html_template,
